...
class MPExperiment(experiment.AbstractIterativeExperiment):
    ...
    def get_dim_in(cfg, sampler):
        ...
        if "TemporalCorrelated" in cfg["sampler"]["type"]:
            dof = cfg["policy"]["args"]["mp"]["args"]["num_dof"]
            return sampler.observation_shape[0] - dof * 2
        else:
            return sampler.observation_shape[0]
    ...
    def dim_policy_out(cfg):
        ...
        """
        mp_type = cfg["policy"]["args"]["mp"]["type"]
        dof = cfg["policy"]["args"]["mp"]["args"]["num_dof"]
        num_basis = cfg["policy"]["args"]["mp"]["args"]["num_basis"]
        learn_tau = cfg["policy"]["args"]["mp"]["args"].get("learn_tau", False)
        learn_delay = cfg["policy"]["args"]["mp"]["args"].get("learn_delay", False)

        if mp_type == "prodmp":
            dim_out = dof * (num_basis + 1)  # weights + goal

            # Disable goal if specified
            if cfg["policy"]["args"]["mp"]["args"].get("disable_goal", False):
                dim_out -= dof
